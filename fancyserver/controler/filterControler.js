...
const filterNumbers = async (req, res) => {
  try {
    const {
      ...
      category,
      page,
      sort, // Add a sort parameter to determine the sorting order
    } = req.query;
    ...
    let sortOption = {};
    if (sort === "highToLow") {
      sortOption = { newPrice: -1 }; // Sort from high to low
    } else if (sort === "lowToHigh") {
      sortOption = { newPrice: 1 }; // Sort from low to high
    }
    ...
    const response = await numberModal
      .find(query)
      .sort(sortOption)
      .skip(skip)
      .limit(pageSize);
